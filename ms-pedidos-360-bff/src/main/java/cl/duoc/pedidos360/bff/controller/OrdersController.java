...
public class BffOrdersController {

    private final OrdersClient ordersClient;

    public BffOrdersController(OrdersClient ordersClient) {
    ...
    public ResponseEntity<List<Object>> listarPedidos(@AuthenticationPrincipal Jwt jwt) {
        ...
        // Regla de negocio: Si es Cliente, solo ve sus propios pedidos (se filtra por sub / preferred_username)
        if (esClientePuro) {
            String clientId = jwt.getClaimAsString("preferred_username");
            if (clientId == null) clientId = jwt.getSubject();
            return ResponseEntity.ok(ordersClient.listarPedidos(clientId));
        }

        // Operador y Admin ven todos los pedidos
        return ResponseEntity.ok(ordersClient.listarPedidos(null));
    }

    @PostMapping
    public ResponseEntity<Object> crearPedido(@RequestBody Object pedido, @AuthenticationPrincipal Jwt jwt) {
        return ResponseEntity.ok(ordersClient.crearPedido(pedido));
    }
    ...
    public ResponseEntity<Object> cambiarEstado(@PathVariable Long id, @RequestBody Map<String, String> body) {
        return ResponseEntity.ok(ordersClient.cambiarEstado(id, body.get("status")));
    }
}